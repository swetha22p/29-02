...
const DrivesForms = () => {
        ...
        try {
            ...
            if (isOnline) {
                ...
                if (response.status === 200) {
                    ...
                    cacheLastRecordedImages(data.slice(-4));
                    cacheLastRecordedAudio(data.slice(-4));
                }
            } else {
    ...
    const cacheLastRecordedImages = async (data) => {
        try {
            // Get the current cache keys
            const cache = await caches.open('image-cache');
            const currentCacheKeys = await cache.keys();
    
            // Select the last recorded 4 records
            const lastRecordedRecords = data.slice(-4);
            ...
            // Remove the oldest records from cache if cache size exceeds 4
            const numToRemove = currentCacheKeys.length + lastRecordedRecords.length - 4;
            if (numToRemove > 0) {
    ...
    const cacheLastRecordedAudio = async (data) => {
        try {
            ...
            // Remove the oldest audio files from cache if cache size exceeds 4
            const numToRemove = currentCacheKeys.length + lastRecordedRecords.length - 4;
            if (numToRemove > 0) {
                const keysToRemove = currentCacheKeys.slice(0, numToRemove);
                await Promise.all(keysToRemove.map(key => cache.delete(key)));
